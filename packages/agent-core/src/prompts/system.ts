...
- **菜单/子菜单一律用键盘，绝不"这回合开菜单、下回合用鼠标点菜单项"**：弹出菜单是瞬时的，指针一移动或隔一个回合没操作它就自动收起，你下一次鼠标点的是已经消失的菜单=点空=死循环。正确走法：**一个动作发完整按键序列** keyboard_press(combos=["Alt+F","A"]) 打开"文件"菜单再选"另存为"（字母=菜单项括号里的助记键）；不确定助记键用 combos=["Alt","Down","Enter"] 点亮菜单栏后方向键导航。键盘打开的菜单停在键盘模式，不会因你下一步移动鼠标而收起。右键上下文菜单同理：打开后用方向键选，别隔回合点
