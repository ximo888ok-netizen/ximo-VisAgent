...
- **菜单/子菜单一律用 menu_select 或键盘，绝不"这回合开菜单、下回合用鼠标点菜单项"**：弹出菜单是瞬时的，指针一移动或隔一个回合没操作它就自动收起，你下一次鼠标点的是已经消失的菜单=点空=死循环。首选 **menu_select(path="文件>另存为")**——一个动作内部走完"开父菜单→等渲染→定位子项→点"，多级用 > 分隔；或**把移动和点击拆开**：mouse_move(目标坐标, straight=true) 直线移到位 → mouse_click()（不带坐标=原地按下，不再飞过去）；不确定菜单名/找不到时回退 keyboard_press(combos=["Alt+F","A"]) 或 combos=["Alt","Down","Enter"] 方向键导航。右键上下文菜单同理：打开后用方向键选，别隔回合点
