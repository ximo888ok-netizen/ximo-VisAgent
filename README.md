...
- **坐标换算**：归一化模式下在执行器入口自动把 0-1000 坐标换算回截图像素，后续 `screenshotToPhysical` 链路不变
...
支持两种坐标系模式，通过环境变量 `AGENT_COORD_MODE` 切换：

**像素模式（默认）**：
- Electron `desktopCapturer` 截图 = 物理像素
- UIA Sidecar Per-Monitor V2 = 物理像素
- 截图叠加 100px 网格刻度线辅助模型精确读数（±5px）

**归一化模式**（`AGENT_COORD_MODE=normalized`）：
- 模型输出 0-1000 范围的归一化坐标
- 执行器入口自动换算：0-1000 → 截图像素 → 物理像素（`screenshotToPhysical`）
- 截图叠加 0-1000 等分网格，标注 100-900 刻度
- 所有反馈给模型的坐标文本自动转回 0-1000 格式（summary / 感知文本 / 候选列表等）
- 预测标记覆盖层自动换算 0-1000 坐标到屏幕物理位置
