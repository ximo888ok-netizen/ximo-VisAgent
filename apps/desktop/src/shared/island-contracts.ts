 ...
 *
 * 频道前缀约定：所有 island:* 频道仅与灵动岛窗口通信（主窗口频道保持
 * 原有 agent:* / approval:* 不变，两者互不干扰）。
 *
 * 对接方式（给后端 Agent 看的接口面）：
 *   1. 主进程向渲染进程推送：
 *        - webContents.send("island:step",            AgentStepEvent)
 *        - webContents.send("island:approval-pending", ApprovalRequest)
 *   2. 渲染进程向主进程发送：
 *        - ipcRenderer.invoke("island:expand")
 *        - ipcRenderer.invoke("island:emergency-stop", { reason })
 *        - ipcRenderer.invoke("island:approval-result", ApprovalResult)
