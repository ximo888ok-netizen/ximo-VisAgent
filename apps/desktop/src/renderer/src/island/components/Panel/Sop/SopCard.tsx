  ...
  return (
    <div data-interactive className="group flex flex-col rounded-xl ig-bg-panel px-3 py-2.5 transition hover:ig-bg-panel-hover">
      <div className="flex items-start justify-between gap-2">
        <span className="min-w-0 flex-1 truncate text-[13px] font-medium t-strong" title={sop.name}>{sop.name}</span>
        <div className="flex shrink-0 items-center gap-0.5">
          <button
            className="rounded-lg px-1.5 py-0.5 text-[12px] t-faint opacity-0 transition group-hover:opacity-100 hover:t-body"
            onClick={onExport}
            title="导出模板（剪贴板/文件）"
          >
            ⤓
          ...
          <button
            className={`rounded-lg px-1.5 py-0.5 text-[12px] transition ${
              confirming
                ? "ig-tag ig-tone-danger ig-fg-danger opacity-100"
                : "t-faint opacity-0 group-hover:opacity-100 hover:ig-fg-danger"
            }`}
            onClick={onDelete}
      ...
      <div className="mt-0.5 line-clamp-2 text-[12px] t-muted" title={sop.description || sop.goalTemplate}>
      ...
      <div className="mt-auto flex items-center justify-between pt-2">
        ...
        <button
          className="island-btn island-btn--primary h-6 px-2.5 text-[12px]"
          onClick={onRun}
        >
          运行
        </button>
