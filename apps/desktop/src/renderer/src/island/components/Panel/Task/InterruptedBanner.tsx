...
export function InterruptedBanner() {
  ...
  return (
    <div className="mx-4 mt-3 flex items-center gap-2 rounded-lg ig-alert ig-tone-warning px-3 py-2.5 island-fade-up">
      <span className="inline-block h-1.5 w-1.5 shrink-0 rounded-full ig-bg-warning" />
      <div className="min-w-0 flex-1 truncate text-[12px] ig-fg-warning" title={task.goal}>
      ...
      <button className="island-btn island-btn--ghost shrink-0 px-2.5 text-[12px]" onClick={dismissInterrupted} data-interactive>
        放弃
      </button>
    </div>
  );
