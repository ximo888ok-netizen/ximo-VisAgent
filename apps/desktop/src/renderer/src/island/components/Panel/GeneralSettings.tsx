    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="flex gap-2">
          <button
            data-interactive
            className={`island-input h-9 flex-1 text-center font-mono text-[13px] ${recording ? "border-[var(--island-input-focus)] text-[var(--p-ice-300)]" : ""}`}
            onClick={() => setRecording(true)}
          >
            {recording ? "按下组合键…（需含 Ctrl/Alt/Shift）" : hotkey}
          </button>
          <button className="island-btn island-btn--ghost h-9 px-3 text-[12px]" onClick={() => setHotkey("Ctrl+Alt+Q")} data-interactive>
            重置默认
          </button>
        </div>
        <p className="mt-1 text-[12px] t-faint">任何时刻按下立即终止 Agent 执行中动作。快速输入热键固定为 Ctrl+Alt+D。</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            data-interactive
          />
          <p className="mt-1 text-[11px] t-faint">超时后任务挂起等待人工，不会自动执行</p>
        </div>
        <div>
          ...
          <input
            ...
            data-interactive
          />
          <p className="mt-1 text-[11px] t-faint">超过后任务失败终止</p>
        </div>
      </div>
      ...
      <div>
        ...
        <input
          ...
          data-interactive
        />
        <p className="mt-1 text-[11px] t-faint">Agent 执行循环的最大步数上限，默认 120 步</p>
      </div>

      {/* 提示音 */}
      <div className="flex items-center justify-between rounded-lg ig-bg-panel px-3 py-2.5">
        <div>
          <div className="text-[12px] t-strong">任务事件提示音</div>
          <div className="text-[11px] t-faint">完成/失败/等待审批时播放系统提示音</div>
        </div>
        <button
          data-interactive
      ...
      <div className="flex items-center justify-between rounded-lg ig-bg-panel px-3 py-2.5">
        <div>
          <div className="text-[12px] t-strong">失败自动重试</div>
          <div className="text-[11px] t-faint">模型/超时/定位类失败时注入原步骤骨架自动重试一次</div>
        </div>
        <button
          data-interactive
      ...
      <div className="flex items-center justify-between rounded-lg ig-bg-panel px-3 py-2.5">
        <div>
          <div className="text-[12px] t-strong">定时任务</div>
          <div className="text-[11px] t-faint">按 cron 计划自动执行 SOP 模板或任务目标</div>
        </div>
        <button
          data-interactive
